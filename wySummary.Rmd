...
`r water_yr` water year (October 
`r water_yr-1` through September `r water_yr`). 
The data for these analyses were collected at the 
`r visitor_center(my_park)` 
...
daily low temperature (TMIN), and daily precipitation accumulation (PRCP).
  
# Temperature
```{r TempData, eval=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
#-- Subset temp data
temp.dat <- wx.dat %>%
...
# Precipitation
```{r PrcpData, eval=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
#-- Subset PRCP data
prcp.dat <- wx.dat %>%
  filter(element == "prcp" & park_unit == my_park & waterYr <= water_yr) %>%
...
`r round(abs(text_refs$PRCP_wy - annual_30yr$PRCP_30yr), 2)` inches 
`r ifelse(text_refs$PRCP_wy - annual_30yr$PRCP_30yr < 0, "less", "more")`
